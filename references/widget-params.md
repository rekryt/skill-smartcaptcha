...
- **НЕДОКУМЕНТИРОВАННЫЙ параметр `theme`** (`'light' | 'dark' | 'auto'`; выверен по исходнику `captcha.js`, июль 2026, в официальной документации отсутствует): уходит в URL iframe виджета; `dark` — принудительно тёмная схема, `auto` — автопереключение по `prefers-color-scheme` **внутри iframe**. Тёмные стили работают при включённой «Динамической цветовой схеме» у капчи. Значение фиксируется в URL при `render()` — смена темы на лету требует `destroy()` + повторного `render()`. Это единственный способ синхронизировать виджет с **ручным** (не системным) переключателем темы сайта: сам `captcha.js` `prefers-color-scheme` в верхнем окне не читает, а проброс CSS `color-scheme` контейнера в iframe зависит от браузера. Как недокументированный — может измениться без предупреждения; сверяй при обновлении `captcha.js`. Полный разбор механики, рецепты и способ перепроверки — [../docs/theming.md](../docs/theming.md).
